...
- [x] **Robust model-output handling** — `extractJsonObject` tolerates ```json markdown fences and surrounding prose (Claude wraps its JSON, which previously caused a hard parse error on otherwise-perfect output). Cloud token limits raised (Claude 2048→8192, OpenAI/Gemini 8192, Ollama 800→4096) so large translations don't truncate mid-JSON. Pure + unit-tested.
- [x] **Expose component-defined handles to the model** (`CUSTOM_NODE_HANDLES` + `handlesForNode`) — nodes whose ports live in their React component (If, Math, Compare, Select, For-Each-Selected, Vector Math, Split Vector, globals, reroute) previously showed "Inputs: —" in the catalog, so the model guessed wrong handle ids (e.g. `exec_true`/`condition` for If → branch + condition silently unwired). The catalog now reports their real handles (`exec_in`/`cond` → `exec_then`/`exec_else`), and the edge validator can catch wrong ones instead of dropping them silently. Unit-tested.
