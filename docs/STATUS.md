...
| §7 | Reverse translation (Blueprints) | Copilot "Blueprint" mode: paste ExtendScript → reconstructed node graph via a reverse-translation prompt; topological left→right auto-layout (no dagre); replace-or-append the canvas; robust edge filter (rejects exec/data mismatches, input fans, duplicates, bad handles); auto-chain recovery (chains unwired action nodes + injects a Start anchor so a translation runs instead of compiling to orphans) |
